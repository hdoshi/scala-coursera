...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    (b.legalNeighbors map {x:(Block, Move) => (x._1, x._2 :: history)}).toStream
  }
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.filter(y => !explored.exists(x => x == y._1))
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    initial match {
      case x #:: xs => {
        val m = for {
          p <- initial
          ne <- newNeighborsOnly(neighborsWithHistory(p._1, p._2), explored)
        } yield ne
