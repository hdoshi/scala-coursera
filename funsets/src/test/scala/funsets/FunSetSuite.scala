...
class FunSetSuite extends FunSuite {
  ...
  test("forall tests all elements satisfy condition") {
    new TestSets {
      val s = union(s1, union(s2, s3))
      assert(forall(s, x => true), "forall true")
      assert(!forall(s, x => x == 2), "forall false")
    }
  }

  test("exists tests one elements satisfy condition") {
    new TestSets {
      val s = union(s1, union(s2, s3))
      assert(!exists(s, x => x == 4), "exists false")
      assert(exists(s, x => x == 2), "exists true")
    }
  }

  test("map creates the right sets") {
    new TestSets {
      val s = union(s1, union(s2, s3))
      val m = map(s, x => x*x)
      assert(contains(m, 1), "Map 1")
      assert(contains(m, 4), "Map 2")
      assert(contains(m, 9), "Map 3")
